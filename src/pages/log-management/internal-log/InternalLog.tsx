...
export const InternalLog: React.FC = () => {
  const { t } = useTranslation();
  const [form] = Form.useForm();
  const history = useHistory();
  const parsedQuery = queryString.parse(history.location.search);
  const { pageSize, pageIndex } = parsedQuery;
  ...
  const getList = useCallback(async () => {
    ...
    const queryParams = [
      ...
      { key: 'path', value: String(filterConditions.path ?? '') },
      { key: 'userName', value: String(filterConditions.userName ?? '') },
      { key: 'statusCode', value: String(filterConditions.statusCode ?? '') },
  ...
  const handleSearch = (value: any) => {
    const newObj = {
      ...
      path: value.path ? String(value.path) : '',
      userName: value.userName ? String(value.userName) : '',
      statusCode: value.statusCode ? String(value.statusCode) : '',
    ...
    <>
      <div style={{ display: isOpenDetail ? 'none' : 'block' }}>
        <Form form={form} initialValues={initialValues} onFinish={handleSearch}>
          <Row gutter={16}>
            <Col span={6}>
              <Form.Item name="searchTerm" label={t('searchKeywords')}>
                <Input prefix={<SearchOutlined />} />
              </Form.Item>
            </Col>
            <Col span={6}>
              <Form.Item name="path" label={t('log.path')}>
                <Input prefix={<SearchOutlined />} />
              </Form.Item>
            </Col>
            <Col span={6}>
          ...
          <Row gutter={16}>
            <Col span={6}>
              <Form.Item name="statusCode" label={t('log.statusCode')}>
                <Input prefix={<SearchOutlined />} />
              </Form.Item>
            </Col>
            <Col span={48}>
